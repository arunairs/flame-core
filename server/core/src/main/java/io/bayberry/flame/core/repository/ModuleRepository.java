...
public class ModuleRepository extends AbstractMongoRepository<Branch, Long> {
    ...
    private WriteResult pushToArchiveModules(Module module) {
        return super.updateFirst(Query.query(Criteria.where(Fields.ID).is(module.getBranchId())),
    ...
    public Optional<Module> get(Long branchId, Long id) {
        Query query = Query.query(Criteria.where(Fields.ID).is(branchId).and("archive.modules._id").is(id));
    ...
    public boolean exists(Long branchId, Long id) {
        return super.exists(Query.query(Criteria.where(Fields.ID).is(branchId).and("archive.modules._id").is(id)));
    }
    ...
    public void delete(Long branchId, Long id) {
        super.updateFirst(Query.query(Criteria.where(Fields.ID).is(branchId).and("archive.modules.moduleOrders").is(id)),
        ...
        super.updateFirst(Query.query(Criteria.where(Fields.ID).is(branchId)),
                new Update().pull("archive.moduleOrders", id)
                        .pull("archive.modules", Query.query(Criteria.where(Fields.ID).is(id)))
